...
const vote = user => amend => type => id => async () => {
...
export const AmendPage = ({ match }) => (
  <DataContext.Consumer>
    {({ get }) => {
      const amend = get('amend')(match.params.id)
      const text = amend && amend.data && get('text')(amend.data.text)

      if (amend && amend.error) return <ErrorPage error={amend.error} />
      if (text && text.error) return <ErrorPage error={text.error} />
